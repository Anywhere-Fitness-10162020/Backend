...
router.post("/register", middleware.verifyRegister, async (req, res) => {
  try {
    ...
    if (newUser.role === instructorCode) {
      const savedUser = await Users.addUser({
        ...newUser,
        password: hash,
        role: "instructor",
      });
      ...
      Users.addUser({ ...newUser, password: hash, role: "attendee" })
        ...
        .catch((userErr) => {
          res
            .status(500)
            .json({
              message: "this username or email is likely not unique",
              userErr,
            });
        });
    }
  ...
  try {
    ...
    if (persistedUser && bcrypt.compareSync(password, persistedUser.password)) {
      ...
      res
        .status(200)
        .json({
          message: `Welcome ${persistedUser.username}. Thanks for being an ${persistedUser.role} today! `,
          token,
          role: persistedUser.role,
        });
    } else {
      res.status(401).json({ message: "You've provided invalid credentials" });
    }
  } catch (error) {
